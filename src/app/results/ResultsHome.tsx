...
const ResultsContent = () => {
    ...
    const [youthOrCommunity, setYouthOrCommunity] = useState<'youth' | 'community'>('youth');
    ...
    const sortResults = useCallback(
        (passedResults?: Result[], passedSortBy?: 'performance' | 'age') => {
            let resultsToSort = passedResults || results;
    ...
    useEffect(() => {
        if (query.trim() === '') {
            ...
            return;
        } else setFilteredResults(sortedResults.filter((athlete) => `${athlete.firstName} ${athlete.lastName}`.toLowerCase().includes(query.toLowerCase())));
    }, [query, sortedResults]);

    useEffect(() => {
        adjustSearchParams(youthOrCommunity, sortBy, eventView);
        sortResults(undefined, sortBy);
    }, [sortBy]);

    useEffect(() => {
        adjustSearchParams(youthOrCommunity, sortBy, eventView.replaceAll(' meters', 'm').replaceAll(' events', ''));
    }, [eventView]);

    useEffect(() => {
        let race = searchParams.get('race');
        if (race === 'youth' || race === 'community') {
            setYouthOrCommunity(race || 'youth');
        } else race = 'youth';
        ...
        axios
            .get(`/api/admin/get/${youthOrCommunity === 'youth' ? 'dpi-youth-participants' : 'dpi-community-participants'}`)
            .then((res) => {
                if (res.status === 200) {
                    setResponse('success');
                    ...
                    sortResults(formattedResults, sort === 'performance' || sort === 'age' ? (sort as 'performance' | 'age') : 'performance');
            ...
            <div className='w-full max-w-[1280px] flex flex-col px-5 my-8'>
                ...
                {isDatePast ? (
                    <div className={`${fredokaLight.className} w-full`}>
                        ...
                        {/* <div className='w-full rounded-full grid grid-cols-2 mt-4 bg-background shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)]'>
                            <button
                                className={`${fredokaBold.className} ${
                                    youthOrCommunity === 'youth' ? 'bg-primary text-white' : ' bg-background text-primary hover:bg-background-secondary'
                            ...
                            <button
                                className={`${fredokaBold.className} ${
                                    youthOrCommunity === 'community' ? 'bg-primary text-white' : ' bg-background text-primary hover:bg-background-secondary'
                                } rounded-full flex justify-center items-center text-sm mid-mobile:text-base p-1.5`}
                                onClick={() => {
                                    adjustSearchParams('community', sortBy, eventView);
                                    setYouthOrCommunity('community');
                                }}
                            >
                                Community
                            </button>
                        ...
                        <div
                            className={`${
                                response === 'success' && `tablet:grid ${eventView !== 'all events' ? 'grid-cols-1' : 'grid-cols-2'}`
                        ...
                        >
                            {response === 'success' ? (
                                events
                                    ...
                                    .map(
                                        (event) =>
                                            filteredResults.filter((result) => result.event === event).length > 0 && (
                                                <div key={event} className='w-full flex flex-col gap-1'>
                                                    ...
                                                    {filteredResults
                                                        ...
                                                        .map((result, index) => (
                                                            <AthleteCard key={result.athleteID} index={index} {...result} />
                                                        ))}
                                                </div>
                                            )
                                    )
...
const AthleteCard = ({ firstName, lastName, performance, unit, age, athleteID, index, position }: Result & { index: number }) => {
    ...
    if (unit === 'inches') performanceString = `${Math.floor(score / 12)}' ${score % 12}"`;
    if (unit === 'meters') {
