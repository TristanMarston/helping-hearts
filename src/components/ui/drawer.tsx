    ...
    ({ className, children, ...props }, ref) => (
        <DrawerPortal>
            ...
            <DrawerPrimitive.Content
                ref={ref}
                className={cn(
                    'fixed inset-x-0 bottom-0 z-50 mt-24 flex h-[20rem] flex-col rounded-t-[10px] border border-slate-200 bg-background dark:border-slate-800 dark:bg-slate-950',
                    className
                )}
                {...props}
            >
                <div className='mx-auto mt-4 h-2 w-[100px] rounded-full bg-background-dark' />
                {children}
            </DrawerPrimitive.Content>
        </DrawerPortal>
    )
...
const DrawerDescription = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Description>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Description>>(
    ({ className, ...props }, ref) => <DrawerPrimitive.Description ref={ref} className={cn('text-sm text-slate-500 dark:text-slate-400', className)} {...props} />
);
